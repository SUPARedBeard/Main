...
class BacklogAdapter(
    ...
    inner class BacklogViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val priorityText = itemView.findViewById<TextView>(R.id.priorityText)


        fun String.capitalizeWords(): String =
            split(" ").joinToString(" ") { it.replaceFirstChar { ch -> ch.uppercaseChar() } }

        fun bind(item: BacklogItem) {
            titleText.text = item.title.capitalizeWords()
            statusText.text = if (item.completed) "Complete" else item.status
            ...
            priorityText.text = "Priority: ${item.priority.capitalizeWords()}"
