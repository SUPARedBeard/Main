...
class ItemListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<FloatingActionButton>(R.id.randomPickFab)?.setOnClickListener {
            if (fullItemList.isNotEmpty()) {
                val randomItem = fullItemList.random()
                showRandomItemDialog(randomItem.title)

            } else {
                AlertDialog.Builder(requireContext())
                    .setTitle("No Items")
                    .setMessage("Your list is empty. Add something first!")
    ...
    private fun applyFilters() {
        ...
        when (currentFilter) {
            ...
            "A-Z" -> filtered = filtered.sortedBy { it.title.lowercase() }
            "Z-A" -> filtered = filtered.sortedByDescending { it.title.lowercase() }
        }
    ...
    private fun showRandomItemDialog(title: String) {
        val titleCase = title.split(" ").joinToString(" ") { word ->
            word.replaceFirstChar { it.uppercase() }
        }

        val message = if (selectedType == "game") {
            "You should play:\n\n$titleCase"
